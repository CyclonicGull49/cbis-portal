  ...
  return (
    <div style={{ maxWidth:900, margin:'0 auto' }}>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:20, flexWrap:'wrap' }}>
        {periodos.map(p => (
          <button key={p} onClick={() => setPeriodo(p)}
            style={{ padding:'7px 20px', borderRadius:20, border:'none', cursor:'pointer', fontFamily:'inherit', fontSize:13, fontWeight:700, background: periodo === p ? '#5B2D8E' : '#f3eeff', color: periodo === p ? '#fff' : '#5B2D8E', boxShadow: periodo === p ? '0 4px 12px rgba(91,45,142,0.3)' : 'none', transition:'all 0.2s' }}>
            Período {p}
          </button>
        ))}
        <button onClick={() => setPeriodo('all')}
          style={{ padding:'7px 20px', borderRadius:20, border:'none', cursor:'pointer', fontFamily:'inherit', fontSize:13, fontWeight:700, background: periodo === 'all' ? '#5B2D8E' : '#f3eeff', color: periodo === 'all' ? '#fff' : '#5B2D8E', boxShadow: periodo === 'all' ? '0 4px 12px rgba(91,45,142,0.3)' : 'none', transition:'all 0.2s' }}>
          Resumen
        </button>
      </div>
      ...
      {loading ? (
        ...
        // Resumen — tabla con NFT por período
        <div style={s.card}>
            ...
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              ...
              <tbody>
                {materias.map(m => {
                  ...
                  const prom = nfts.filter(n => n !== null).length
                    ? nfts.filter(n => n !== null).reduce((a, b) => a + b, 0) / nfts.filter(n => n !== null).length
                    : null
                  return (
                    <tr key={m.id}>
                      <td style={{ ...s.td, textAlign:'left', fontWeight:700, color:'#0f1d40' }}>
                        {m.nombre}
        ...
        <div style={s.card}>
          ...
          <div style={{ padding:'12px 16px', background:'#fafafa', borderTop:'1px solid #f3eeff', display:'flex', gap:16, flexWrap:'wrap' }}>
            {comps.map(c => (
              <span key={c} style={{ fontSize:11, color:'#9ca3af', fontWeight:500 }}>
                <strong style={{ color:'#6b7280' }}>{COMP_LABELS[c]}</strong> {Math.round((PESOS[c] || 0) * 100)}%
              </span>
            ))}
            <span style={{ fontSize:11, color:'#5B2D8E', fontWeight:700, marginLeft:'auto' }}>NFT = suma ponderada</span>
          </div>
        </div>
      )}
    </div>
