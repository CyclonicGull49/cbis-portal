...
function DashboardHome() {
  ...
  async function cargarStats() {
    ...
    const [{ data: estudiantes }, { data: pagosHoy }, { data: pagosMes }, { data: pagosAnulados }, { data: cobros }, { data: pagosRecientes }, { count: notasCount }, { count: materiasGradoCount }] = results
    ...
    <div style={{ maxWidth: '100%', fontFamily: 'Plus Jakarta Sans, system-ui, sans-serif' }}>
      ...
      {verDocente && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 16 }}>
          <KpiCard icon={<IcoDocente />}  label="Materias asignadas"  val={stats.docenteMaterias}                          sub={stats.docenteGrado?.nombre || 'Docente'} color="#5B2D8E" acento />
          <KpiCard icon={<IcoNotas />}    label="Notas registradas"   val={loading ? '...' : stats.notasRegistradas.toLocaleString()} sub={`año ${new Date().getFullYear()}`}    color="#0e9490" />
        </div>
      )}
        ...
        <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 20px rgba(61,31,97,0.09)', overflow: 'hidden' }}>
          ...
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '48px 0' }}>
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#d8c8f0" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginBottom: 10 }}>
                <rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/>
              </svg>
