...
const SUBTIPOS = {
  ausencia:          { label: 'Ausencia',           color: '#dc2626', bg: '#fef2f2', border: '#fca5a5' },
  retiro_anticipado: { label: 'Retiro anticipado',  color: '#d97706', bg: '#fffbeb', border: '#fcd34d' },
  llegada_tarde:     { label: 'Llegada tarde',      color: '#0e9490', bg: '#e0f7f6', border: '#5eead4' },
}
...
export default function Permisos() {
  ...
  async function cargarPermisos() {
    const { data } = await supabase.from('permisos')
      .select(`*, 
        ...
        registrador:perfiles!permisos_registrado_por_fkey(nombre, apellido),
        aprobador:perfiles!permisos_aprobado_por_fkey(nombre, apellido)
      `)
      .eq('año_escolar', year)
  ...
  async function crear() {
    if (!form.estudiante_id || !form.motivo.trim()) {
      toast.error('Estudiante y motivo son obligatorios'); return
    }
    if (form.subtipo === 'retiro_anticipado' && !form.quien_retira.trim()) {
      toast.error('Indica quién retira al estudiante'); return
    }
    setGuardando(true)
    const { error } = await supabase.from('permisos').insert({
      ...
      estado:        esRecepcion ? 'aprobado' : 'pendiente',
      registrado_por: perfil.id,
      aprobado_por:  esRecepcion ? perfil.id : null,
      aprobado_en:   esRecepcion ? new Date().toISOString() : null,
      año_escolar:   year,
    })
  ...
  async function aprobar(permiso) {
    ...
    else { toast.success('Permiso aprobado — retiro autorizado'); cargarPermisos(); setModalDetalle(null) }
    setProcesando(null)
  }
  ...
  async function rechazar(permiso) {
    ...
    if (error) toast.error('Error')
    else { toast.success('Permiso rechazado'); cargarPermisos(); setModalDetalle(null) }
    setProcesando(null)
  }
  ...
  const permisosFiltrados = permisos.filter(p => {
    ...
    const matchTab = tabActiva === 'todos' ? true
      : tabActiva === 'hoy' ? p.fecha === hoy()
      : tabActiva === 'pendientes' ? p.estado === 'pendiente'
      : tabActiva === 'retiros' ? p.subtipo === 'retiro_anticipado'
      : true
  ...
  return (
    <div style={{ fontFamily: 'Plus Jakarta Sans, system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ color: '#3d1f61', fontSize: 22, fontWeight: 800, marginBottom: 4, letterSpacing: '-0.5px' }}>Permisos</h1>
          <p style={{ color: '#b0a8c0', fontSize: 13 }}>
            ...
            {retirosHoy > 0 && <span style={{ color: '#dc2626', fontWeight: 700 }}>{retirosHoy} retiro{retirosHoy !== 1 ? 's' : ''} hoy · </span>}
            {permisos.length} total este año
          </p>
        </div>
        ...
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setTabActiva(tab.id)}
            ...
            {tab.id === 'pendientes' && pendientesCount > 0 && (
              <span style={{ background: tabActiva === 'pendientes' ? 'rgba(255,255,255,0.3)' : '#fef9c3', color: tabActiva === 'pendientes' ? '#fff' : '#92400e', fontSize: 10, fontWeight: 800, padding: '1px 6px', borderRadius: 10 }}>
                {pendientesCount}
              </span>
            )}
            {tab.id === 'retiros' && retirosHoy > 0 && (
              <span style={{ background: tabActiva === 'retiros' ? 'rgba(255,255,255,0.3)' : '#fef2f2', color: tabActiva === 'retiros' ? '#fff' : '#dc2626', fontSize: 10, fontWeight: 800, padding: '1px 6px', borderRadius: 10 }}>
                {retirosHoy}
              </span>
            )}
          </button>
      ...
      {modalNuevo && (
        <div style={s.modalBg} onClick={() => { setModalNuevo(false); resetForm() }}>
          <div style={{ ...s.modalBox, maxWidth: 500 }} onClick={e => e.stopPropagation()}>
            <h2 style={{ color: '#3d1f61', fontSize: 17, fontWeight: 800, marginBottom: 20 }}>Registrar permiso</h2>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                {Object.entries(SUBTIPOS).map(([k, v]) => (
                  <button key={k} onClick={() => setForm(f => ({ ...f, subtipo: k }))}
                    style={{ padding: '10px 8px', borderRadius: 10, border: `2px solid ${form.subtipo === k ? v.color : '#e5e7eb'}`, background: form.subtipo === k ? v.bg : '#fff', color: form.subtipo === k ? v.color : '#6b7280', fontWeight: form.subtipo === k ? 700 : 500, fontSize: 11, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'center' }}>
                    {v.label}
                  </button>
                ))}
              </div>
            ...
            <div style={s.field}>
              ...
              <textarea style={{ ...s.inputFull, minHeight: 80, resize: 'vertical' }}
                placeholder={
                  form.subtipo === 'ausencia' ? 'Motivo de la ausencia...' :
                  form.subtipo === 'retiro_anticipado' ? 'Motivo del retiro anticipado...' :
            ...
            {esRecepcion && (
              <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 10, padding: '8px 12px', marginBottom: 14, fontSize: 12, color: '#166534', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                    <div style={{ width: 44, height: 44, borderRadius: 12, background: subtipo.bg, border: `1.5px solid ${subtipo.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <span style={{ width: 16, height: 16, borderRadius: '50%', background: subtipo.color, display: 'block' }} />
                    </div>
                    <div>
                      <div style={{ display: 'flex', gap: 6, marginBottom: 4 }}>
                        <span style={{ fontSize: 11, fontWeight: 700, color: subtipo.color, background: subtipo.bg, padding: '2px 10px', borderRadius: 10 }}>{subtipo.label}</span>
                      ...
                      <div style={{ fontSize: 15, fontWeight: 800, color: '#3d1f61' }}>
                        {p.estudiantes?.apellido}, {p.estudiantes?.nombre}
                      </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                      ...
                      {p.hora_retiro && (
                        <div>
                          ...
                          <div style={{ fontSize: 13, fontWeight: 600, color: '#374151' }}>{p.hora_retiro}</div>
                        </div>
                      )}
                      {p.quien_retira && (
                  ...
                  <div style={{ fontSize: 11, color: '#b0a8c0', marginBottom: p.estado === 'pendiente' && esRecepcion ? 16 : 20 }}>
                    Registrado por {p.registrador?.nombre} {p.registrador?.apellido} · {formatTs(p.creado_en)}
                    {p.aprobador && <span> · {p.estado === 'aprobado' ? 'Aprobado' : 'Rechazado'} por {p.aprobador.nombre} {p.aprobador.apellido}</span>}
                  </div>
